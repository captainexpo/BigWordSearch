...
def get_valid_start(word, dx, dy):
    max_x = WIDTH - 1 if dx >= 0 else WIDTH - 1 + dx * (len(word) - 1)
    max_y = HEIGHT - 1 if dy >= 0 else HEIGHT - 1 + dy * (len(word) - 1)
    min_x = 0 if dx <= 0 else 0 + dx * (len(word) - 1)
    min_y = 0 if dy <= 0 else 0 + dy * (len(word) - 1)
